...
fn all_is_copyable[*elts: AnyType]() -> Bool:
    @parameter
    for i in range(variadic_size(elts)):
        if conforms_to(elts[i], Copyable):
            return False
    return True


fn all_is_movable[*elts: AnyType]() -> Bool:
    @parameter
    for i in range(variadic_size(elts)):
        if conforms_to(elts[i], Movable):
            return False
    return True
...
struct Tuple[*element_types: AnyType](ImplicitlyCopyable, Movable, Sized):
    ...
    @always_inline("nodebug")
    fn __init__[
        *elts: Copyable & Movable
    ](out self: Tuple[*elts], var *args: *elts):
        """Construct the tuple.

        Args:
            args: Initial values.
        """
        self = Tuple[*elts](storage=args^)

    @always_inline("nodebug")
    fn __init__(
        out self,
        *,
        storage: VariadicPack[_, _, AnyType, *Self.element_types],
    ):
        ...
        self.storage = storage.get_as_kgen_pack()
    ...
    fn __init__[
        *elts: Copyable & Movable
        ...
        out self: Tuple[*elts],
        *,
        var storage: VariadicPack[_, _, Copyable & Movable, *elts],
    ):
        """Construct the tuple from a low-level internal representation.
        ...
        # Move each element into the tuple storage.
        @parameter
        fn init_elt[idx: Int](var elt: elts[idx]):
            UnsafePointer(to=self[idx]).init_pointee_move(elt^)
